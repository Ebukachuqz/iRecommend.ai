...
def fetch_user_taste_vectors_for_category(client: Any, category: str) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    start = 0
    while True:
        end = start + PAGE_SIZE - 1
...
def compute_category_readiness(client: Any, category: str) -> dict[str, Any]:
    ...
    taste_vectors = fetch_user_taste_vectors_for_category(client, category)
    users_with_personas = {str(row["user_id"]) for row in personas if row.get("user_id")}
    users_with_taste_vectors = {str(row["user_id"]) for row in taste_vectors if row.get("user_id")}
    ...
    users_with_persona_taste_and_task_b_holdout = [
        user_id
        for user_id, counts in by_user.items()
        if user_id in users_with_personas
        and user_id in users_with_taste_vectors
    ...
    task_b_ready = len(users_with_persona_taste_and_task_b_holdout) > 0 and product_embeddings_count > 0
    ...
    return {
        ...
        "user_taste_vectors": len(taste_vectors),
        ...
        "users_with_persona_taste_and_task_b_holdout": len(users_with_persona_taste_and_task_b_holdout),
