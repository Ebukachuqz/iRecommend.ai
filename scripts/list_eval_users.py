...
from scripts.check_category_readiness import (
    ...
    fetch_user_taste_vectors_for_category,
)
...
def list_eval_users(
    ...
    *,
    ...
    require_taste_vector: bool = False,
    task: str = "both",
) -> list[dict[str, Any]]:
    ...
    users_with_taste = {str(row["user_id"]) for row in taste_vectors if row.get("user_id")}

    rows: list[dict[str, Any]] = []
    for user_id in sorted(by_user):
        counts = by_user[user_id]
        has_persona = user_id in users_with_personas
        has_taste_vector = user_id in users_with_taste
        row = {
            "user_id": user_id,
            "category": category,
    ...
    def allowed(item: dict[str, Any]) -> bool:
        if require_persona and not item["has_persona"]:
            return False
        if require_taste_vector and not item["has_taste_vector"]:
            return False
        if task == "task_a":
...
def main() -> None:
    ...
    parser.add_argument("--require-taste-vector", action="store_true")
    ...
    rows = list_eval_users(
        ...
        require_taste_vector=args.require_taste_vector,
        task=args.task,
    )
    for row in rows:
        print(row)
